...
st.markdown(user_template.format(msg="Hi Bot!"), unsafe_allow_html=True)
st.markdown(bot_template.format(msg="Hi Hooman!"), unsafe_allow_html=True)
if st.button("Submit"):
	...
	for index, message in enumerate(st.session_state.chat_history):
		...
		if st.session_state[f"show_docs_{index}"]:
			with st.expander("Source Documents"):
				doc = message.get('context_docs', [])[0]
				source = doc.to_json()
				st.write(f"Source: {source['kwargs']['metadata']['source']}")
